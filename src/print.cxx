...
void print_iteration(int iter,ALLDATA &A)
{
    ...
    for(i=0;i<NP;i++) 
    {
        if ((A.attr[i] & P_ACTIVE) && (A.attr[i] & P_ROTDIF) && ((A.attr[i] & P_LOCAL) == 0)) 
        {
...
void show_models (ALLDATA &A, char *XMGR_PREFIX)
{
    ...
    { 
        if (A.attr[i] & P_ROTDIF) 
        {
            ...
            printf("RESIDUE %-5d %5s %8.04f\n", 
                A.num[r], A.pid[i], unit*gsl_matrix_get(A.vP[i],r,b));
        }
    }
    ...
    if (D & _AXIAL_) 
    {
        printf("RESIDUE %-5d alpha %8.04f\n", A.num[r], A.alpha[r]*180.0/M_PI);
    }
    
    /* additional information for anisotropic */
    if (D & _ANISOTROPIC_)
    {
        printf("RESIDUE %-5d    xi %8.04f\n", A.num[r], A.xi[r]);
        printf("RESIDUE %-5d    yi %8.04f\n", A.num[r], A.yi[r]);
        printf("RESIDUE %-5d    zi %8.04f\n", A.num[r], A.zi[r]);
    }
      
    /* model parameters */
    for (m=1; m<=NM; m++) 
    {
        if (gsl_matrix_get(A.x2,r,m) >=0 )  /* only for fit models */
        {
            if (m == b) 
            {
                printf(">"); /* best model */
            ...
            if (gsl_matrix_get(A.dof,r,m) > 0) /* chi-square */
            {
                if (gsl_cdf_chisq_Q (gsl_matrix_get(A.x2,r,m), gsl_matrix_get(A.dof,r,m)) >= critx2)
                {
    ...
    if (MC > 0) 
    {
        ...
        printf("S2s %5.3f %5.3f ", gsl_matrix_get(A.aP[_S2s_],r,b),gsl_matrix_get(A.dP[_S2s_],r,b));
        printf("S2f %5.3f %5.3f ", gsl_matrix_get(A.aP[_S2f_],r,b),gsl_matrix_get(A.dP[_S2f_],r,b));
        printf("te %6.3f %6.3f ",  gsl_matrix_get(A.aP[_te_],r,b),gsl_matrix_get(A.dP[_te_],r,b));
    ...
    for(i=0;i<NP;i++) 
    {
        A.p[i] = gsl_matrix_get(A.vP[i],r,b);
    }
    R1R2NOE(&A,fit,NULL);
    ...
    if (MC > 0) 
    {
        for(i=0;i<NP;i++) 
        {
            A.p[i] = gsl_matrix_get(A.aP[i],r,b);
        }
        R1R2NOE(&A,sim,NULL);
    }
    ...
    for(f=0; f<A.NF; f++) 
    {
        ...
        if (MHz > 0) 
        {
            ...
            {
                printf("%7.4f %7.4f ", gsl_matrix_get(A.Y,r,3*f+0),gsl_matrix_get(A.SIG,r,3*f+0));
            }
            else
            {
                printf("%7s %7s "," "," ");
            ...
            {
                printf("%7.4f %7.4f ", gsl_matrix_get(A.Y,r,3*f+1),gsl_matrix_get(A.SIG,r,3*f+1));
            }
            else
            {
                printf("%7s %7s "," "," ");
